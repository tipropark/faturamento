'use client';
// Auditoria de Faturamento - movida de /metas para /faturamento
import React, { useState, useEffect, useCallback } from 'react';
import { useRouter } from 'next/navigation';
import {
  AlertCircle, Eye, Wallet,
  Search, Filter, TrendingUp, Target,
  ChevronRight, RefreshCcw, ArrowLeft
} from 'lucide-react';
import {
  ...
  return (
    <div className="page-container" style={{ padding: '1.25rem', background: '#F8FAFC', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', background: 'white', padding: '1.25rem 2rem', borderRadius: '16px', border: '1px solid #E2E8F0', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '36px', height: '36px', borderRadius: '10px', border: '1px solid #E2E8F0', background: 'white', cursor: 'pointer', color: '#475569' }}
          >
            <ArrowLeft size={18} />
          </button>
          <div>
            ...
            <h1 style={{ fontSize: '1.6rem', fontWeight: 900, color: '#0F172A', marginTop: '0.2rem', letterSpacing: '-1px' }}>Auditoria de Faturamento</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.25rem', marginBottom: '1.5rem' }}>
         {[
            { label: 'Total de Alertas', value: stats.abertos + (alertas.length || 0), color: '#F97316', icon: <AlertCircle size={22} />, bg: '#FFF7ED' },
