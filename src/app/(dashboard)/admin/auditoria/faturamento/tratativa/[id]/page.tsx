              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxHeight: '600px', overflowY: 'auto', paddingRight: '0.5rem' }}>
                 {diarias.map((d: any, idx: number) => (
                    <div key={idx} style={{ padding: '0.85rem', borderRadius: '12px', background: d.data_referencia === alerta.data_referencia ? 'var(--danger-bg)' : 'var(--bg-app)', border: `1px solid ${d.data_referencia === alerta.data_referencia ? 'var(--danger)' : 'var(--border-color)'}` }}>
                       <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem' }}><span style={{ fontSize: '0.75rem', fontWeight: 800 }}>{new Date(d.data_referencia + 'T12:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}</span><span style={{ fontSize: '0.75rem', fontWeight: 700, color: d.desvio_ritmo >= 0 ? 'var(--success)' : 'var(--danger)' }}>{d.desvio_ritmo >= 0 ? '+' : ''}{formatMoeda(d.desvio_ritmo)}</span></div>
                       <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: '0.65rem', color: 'var(--gray-500)' }}>Realizado:</span><span style={{ fontSize: '0.75rem', fontWeight: 700 }}>{formatMoeda(d.valor_realizado_dia)}</span></div>
                    </div>
                 ))}
              </div>
           </div>
