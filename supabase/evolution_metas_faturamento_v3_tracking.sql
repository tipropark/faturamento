-- LEVE ERP — Evolução: Acompanhamento Gerencial de Metas (v3)
-- Focada em rastreabilidade diária e projeção de fechamento.

-- 1. ADAPTAÇÃO DAS TABELAS EXISTENTES
...
-- Garante coluna de expectativa no consolidado mensal
...
-- Limpa registros órfãos ou duplicados para garantir a constraint UNIQUE
DELETE FROM public.metas_faturamento_apuracoes a
WHERE a.id NOT IN (
    SELECT MAX(id) 
...
-- Adiciona Unique Constraint para permitir UPSERT na apuração consolidada
DO $$ 
BEGIN
...
BEGIN
    ...
    -- 3. Limpar apurações antigas daquela meta (preservando ajustes manuais e observações se existirem)
    -- Em um sistema real, faríamos um UPSERT. Aqui vamos simplificar limpando apenas o que não foi "confirmado".
    ...
    WHILE v_data_cursor <= v_data_fim AND v_data_cursor <= CURRENT_DATE LOOP
        ...
        -- Buscar realizado (Global ou Operação) - AGORA USANDO APENAS RECEITAS (FATURAMENTO BRUTO)
        IF rec_meta.operacao_id IS NULL THEN
            SELECT 
                SUM(COALESCE(total_receita_ajustada, 0) + 
                    COALESCE(total_avulso_ajustado, 0) + 
                    COALESCE(total_mensalista_ajustado, 0))
            INTO v_realizado_dia
            FROM public.faturamento_resumo_diario
            WHERE data_referencia = v_data_cursor;
        ELSE
            SELECT 
                (COALESCE(total_receita_ajustada, 0) + 
                 COALESCE(total_avulso_ajustado, 0) + 
                 COALESCE(total_mensalista_ajustado, 0))
            INTO v_realizado_dia
            FROM public.faturamento_resumo_diario
        ...
        -- Lógica de Status Diário (comparando realizado_dia com meta_diaria)
        IF v_realizado_dia = 0 THEN
            v_status_dia := 'sem_dados';
        ELSIF v_realizado_dia >= v_meta_diaria_calculada * (1 + v_tolerancia) THEN
            v_status_dia := 'acima_da_meta';
        ELSIF v_realizado_dia >= v_meta_diaria_calculada * (1 - v_tolerancia) THEN
            v_status_dia := 'dentro_da_meta';
        ELSE
            v_status_dia := 'abaixo_da_meta';
        END IF;

        -- UPSERT na tabela diária (preservando o que já existia se houver)
        INSERT INTO public.metas_faturamento_apuracoes_diarias (
            ...
            valor_meta_parcial_esperada, meta_planejada,
            desvio_dia, desvio_acumulado,
    ...
    -- 5. CÁLCULO DE PROJEÇÃO E STATUS MENSAL
    -- Projeção = (Média Diária) * Dias do Mês
    IF v_dias_passados > 0 THEN
        v_projecao := (v_acumulado / v_dias_passados) * v_dias_mes;
    ELSE
        v_projecao := 0;
    END IF;

    -- Status Mensal (Ritmo)
    IF v_acumulado >= v_meta_parcial_esperada THEN
        v_status_mensal := 'no_ritmo';
    ELSIF v_acumulado >= v_meta_parcial_esperada * 0.90 THEN
        v_status_mensal := 'levemente_atrasada';
    ELSIF v_acumulado >= v_meta_parcial_esperada * 0.75 THEN
        v_status_mensal := 'em_recuperacao';
    ELSE
        v_status_mensal := 'critica';
    END IF;

    -- 6. UPSERT NO CONSOLIDADO MENSAL
...
-- 7. REPROCESSAMENTO EM MASSA (Retroativo)
DO $$
DECLARE
    v_m RECORD;
BEGIN
