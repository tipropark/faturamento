...
async function checkTotalCount() {
  try {
    ...
    dataLimiteBuffer.setDate(dataLimiteBuffer.getDate() - 14);
    const isoLimiteBuffer = dataLimiteBuffer.toISOString();

    const { count, error } = await supabase
      .from('faturamento_movimentos')
      .select('*', { count: 'exact', head: true })
      .or(`data_saida.gte.${isoLimiteBuffer},criado_em.gte.${isoLimiteBuffer}`);

    console.log(`Total movements in the last 14 days: ${count}`);
